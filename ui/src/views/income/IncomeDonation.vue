<template>
  <div>
    ...
    <form v-on:submit.prevent="submitIncome()" class="row g-3 text-start">
      ...
      <div class="col-md-6">
        <label for="itemName" class="form-label fw-bold">Item</label>
        <input id="itemname" type="text" class="form-control" placeholder="Describe item" v-model="item" required>
      </div>
      <div class="col-md-6">
        <label for="amountDollars" class="form-label fw-bold">Approx. Value</label>
  ...
  methods: {
    async submitIncome() {
      ...
      const response = await fetchWrapperTXT(`/api/v2/income`, {
        ...
        body: JSON.stringify({committee:this.committee,source:this.source,item:this.item,
