...
router.get("/:commID/categories", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeCategories(committee_lut[req.params.commID][0]);
...
router.get("/:commID/balance", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.account.getUserApprovals(req.context.request_user_id, committee_lut[req.params.commID][0]);
        if (results.length === 0) {
            return res.status(404).send("Invalid committee value");
        }
    } catch (err) {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeBalance(committee_lut[req.params.commID][0]);
...
router.get("/:commID/budget/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeBudgetTotals(committee_lut[req.params.commID][0], req.params.year);
        if (results.length === 0) {
            return res.status(404).send("Invalid committee value");
        }
        return res.status(200).send(results[0]);
...
router.get("/:commID/expensetotal/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeExpenseTotals(committee_lut[req.params.commID][0], req.params.year);
        if (results.length === 0) {
            return res.status(404).send("Invalid committee value");
        }
        return res.status(200).send(results[0]);
...
router.get("/:commID/incometotal/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeIncomeTotals(committee_lut[req.params.commID][0], req.params.year);
        if (results.length === 0) {
            return res.status(404).send("Invalid committee value");
        }
        return res.status(200).send(results[0]);
...
router.get("/:commID/purchases/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteePurchases(committee_lut[req.params.commID][0], req.params.year);
        return res.status(200).send(results);
    } catch (err) {
        logger.error(err.stack);
        return res.status(500).send("Internal Server Error");
...
router.get("/:commID/income/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeIncome(committee_lut[req.params.commID][0], req.params.year);
        return res.status(200).send(results);
    } catch (err) {
        logger.error(err.stack);
        return res.status(500).send("Internal Server Error");
...
router.get("/:commID/summary/:year?", async(req, res) => {
    ...
    try {
        const [results, _] = await req.context.models.committee.getCommitteeBudgetSummary(committee_lut[req.params.commID][0], req.params.year);
        return res.status(200).send(results);
    } catch (err) {
        logger.error(err.stack);
        return res.status(500).send("Internal Server Error");
